import numpy as np
from src.distributions import TfLogNormal, TfNormal, TfConstant,TfTruncatedNormal, TfKumaraswamy
...
class DistributionDescription(object):
    def __init__(self, name, class_type, defaults):
        ...
        if class_type == TfNormal or class_type == TfLogNormal:
            self.nbr_free_params = 2 #确实只需要两个自由参数即可唯一描述LogNormal或者Normal分布
            ...
            if "prec" in defaults:
                ...
                if (defaults["sigma"] is not None) and (defaults["sigma"].__class__ != str):  # 确保对应sigma的值不是字符串
                    init_prec = 1.0 / np.square(defaults["sigma"])  # precision是sigma的倒数
                    init_log_prec = np.log(init_prec)
            else:
...
def instantiate_from_specs(name, specs, conditioning):
    try:  # catch conditions dict and skip
        ...
        return None
    if sd == "Normal":
        mu = default_get_value(specs, "mu", 0.0) #字典{'distribution':,'mu':,'sigma':}
        ...
        return DistributionDescription(name, TfNormal, s) #TfNormal(**s) #TfNormal comes from distribution
    if sd == "LogNormal":
...
class DotOperatorParams(object):
    ...
    def add_from_spec(self, name_of_param, spec_of_param, conditioning):
        if hasattr(self, name_of_param):
            ...
            setattr(self, name_of_param, instantiate_from_specs(name_of_param, spec_of_param, conditioning)) #setattr给一个对象的属性赋值，把言语描述的分布实例化
...
class Parameters(object):
    ...
    def get_parameter_counts(self):
        n_local = self.l.get_parameter_counts() if hasattr(self, "l") else 0 #self.l是一个DotOperator
    ...
    def add_shared(self, p):
        self.s = p #把DotOperatorParams的实例p (parameters)传给Parameter类的属性s
    ...
    def load(self, params_dict):
        ...
        self.load_global(params_dict) #global的参数没有-->没有g
        self.load_global_cond(params_dict) #global_conditioned的参数也没有-->没有g_c
        self.load_local(params_dict)
        self.load_constant(params_dict)

    def load_shared_distributions(self, params_dict):
        keyword = 'shared' #定义关键词 方便后续操作
        p = DotOperatorParams()
        ...
        keyword_dict = params_dict[keyword] #字典
        conditioning = None
    ...
    def load_constant(self, params_dict, keyword="constant"):
        ...
        for k, v in keyword_dict.items():
            if k == 'conditioning':
                continue
            p.add_from_spec(k, {'distribution':'Constant', 'value':v}, conditioning) #这里设定了Constant的distribution
        self.add_constant(p)
    ...
    def load_local(self, params_dict, keyword="local"):
        ...
        conditioning = None
        if 'conditioning' in keyword_dict:
            conditioning = keyword_dict['conditioning'] #{'devices':True,'treatments':False}

        for k, v in keyword_dict.items():
            if k == 'conditioning':
                pass
            elif self.is_shared(v['distribution']): #对象Parameters的s属性是否为空，然后Parameters.s是否有属性'standard_normal'
                p.add_from_spec(k, params_dict['shared'][v['distribution']], conditioning) #distribution就叫做Normal
