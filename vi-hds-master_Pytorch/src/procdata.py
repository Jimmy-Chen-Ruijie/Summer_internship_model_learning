...
def apply_defaults(spec):
    ndevices = len(spec["devices"])  # 长度为6
    params = {
        'groups': {'default': [0] * ndevices},  # [0,0,0,0,0,0]
    ...
    for k in spec:
        params[k] = spec[k]  # 把字典spec中的键值对添加到params字典中 （如果params和spec中的key一样，那么value改成spec中的）
    return params
...
class ProcData:
    ...
    def __init__(self, data_settings):
        ...
        self.device_depth = sum(
            [depth(cm.values()) for k, cm in self.component_maps.items()]) # the total number of devices involved in the experiment, to create multi-hot vector
    ...
    def load_multiple(self, data_dir, data_file, devices, conditions):
        ...
        data_full_path = os.path.join('..', data_full_path)  # pytest 和 test_ode_solver 对于相对路径的格式要求不太一样
        # print(data_full_path)
        loaded = pd.read_csv(data_full_path, sep=',', na_filter=False)
        timesall = loaded.iloc[0, 5:]  # times of the observations 一行就是series
        obs_rows = loaded.iloc[1:, :]  # observation rows 多行data就是dataframe
...
class Dataset(object):
    ...
    def __init__(self, data, D: OrderedDict, original_data_ids: List[int],
        ...
        self.X = D['X']
        c_panda = pd.DataFrame(D['C']['values'], columns=D['C']['columns'])  # columns指定列名(sample属性的个数)，每一行是一个具体的sample
        # Numpy array of float, shape e.g. (234,3)
        self.C = D['C']['values'].astype(np.float32)
        # Numpy array of float, shape e.g. (234,2)
        self.treatments = c_panda[self.conditions].values.astype(np.float32)  # 根据列名截取出treatments这个DataFrame
        # print("Log + 1 transform conditions")
        self.treatments = np.log(1.0 + self.treatments)  # log是以e为底的自然对数
        self.n_treatments = len(self.treatments)  # 234个samples
        # Numpy array of float, shape e.g. (234,)
        self.devices = c_panda['Device'].values.astype(int)
        # more numpy data
        # Numpy array of float, shape e.g. (234,86,3)
        self.delta_X = np.diff(self.X, axis=-1)  # 沿着最后一个axis作离散插值
        # Set up set.dev_1hot, numpy array of float, shape e.g. (234,7)
        self.dev_1hot = None
    ...
    def create_feed_dict(self,u_value, device):
        return {'x_obs': torch.Tensor(self.X).to(device),
                'dev_1hot': torch.Tensor(self.dev_1hot).to(device),
                'conds_obs': torch.Tensor(self.treatments).to(device),
                'beta': torch.Tensor([1.0]).to(device),
                'u': torch.Tensor(u_value).to(device)}

    def create_feed_dict_for_index(self, index, beta_val, u_value, device): #slicing via randomized index
        return {'x_obs': torch.Tensor(self.X[index]).to(device),  # (36,86,4)
                'dev_1hot': torch.Tensor(self.dev_1hot[index]).to(device),  # (36,7)
                'conds_obs': torch.Tensor(self.treatments[index]).to(device),  # (36,2)
                'beta': torch.Tensor([beta_val]).to(device),
                'u': torch.Tensor(u_value).to(device)}  # (36,200,4)

    def scale_data(self):
        n_outputs = np.shape(self.X)[2]  # 总共4个output channel
        if self.procdata.normalize is None:
            self.scales = [np.max(self.X[:, :, i]).astype(np.float32) for i in
                           range(n_outputs)]  # 每个output channel都找出最大值
        else:
        ...
        for i, scale in enumerate(self.scales):
            ...
            if self.procdata.subtract_background:  # [:, np.newaxis]在最后一位添加了一个axis
                self.X[:, :, i] -= np.min(self.X[:, :, i], axis=1)[:,
                                   np.newaxis]  # 每个output channel的每个时间sample的最小值 这样每个时间序列都能为正数
